...
def main():
    ...
    ap.add_argument('--target-lufs', type=float, default=-18)
    ...
    try:
        ...
        base = os.path.splitext(os.path.basename(args.inp or 'track'))[0] if args.inp else 'track'
        name_map = {'Drums': '01_Drums_stem.wav', 'Bass': '02_Bass_stem.wav', 'Other': '03_Other_stem.wav', 'Lead_RVC': '04_Lead_RVC_processed.wav'}
        guess = name_map.get(base, base + '_processed.wav')
        out_path = args.out or os.path.join(args.outdir, guess)
        if not os.path.exists(out_path) or args.force:
            open(out_path, 'wb').close()
        sc = sidecar(out_path, stage=os.path.basename(__file__).replace('.py',''), preset=args.preset, overrides=ov)
        sc['input'] = os.path.abspath(args.inp) if args.inp else None
        write_json(out_path + '.json', sc)
        if args.log_json:
            with open(args.log_json, 'a', encoding='utf-8') as lj:
                lj.write(json.dumps({'stage': sc['stage'], 'output': sc['output'], 'ts': sc['timestamp']}) + '\n')
        return 0
